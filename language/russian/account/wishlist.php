<?php
// Heading 
$_['heading_title'] = 'Мои желания';

// Text
$_['text_account']  = 'Мой профиль';
$_['text_instock']  = 'В наличии';
$_['text_wishlist'] = 'Мои желания (%s)';
$_['text_login']    = 'Необходимо войти в <a href="%s">Мой профиль</a> или <a href="%s">создать учетную запись</a>, чтобы сохранить товар <a href="%s">%s</a> в свои <a href="%s">закладки</a>!';
$_['text_success']  = 'Товар <a href="%s">%s</a> добавлен в <a href="%s">закладки</a>!';
$_['text_remove']   = 'Закладки успешно обновлены!!';
$_['text_empty']    = 'Ваши закладки пусты';

// Column
$_['column_image']  = 'Изображение';
$_['column_name']   = 'Наименование товара';
$_['column_model']  = 'Модель';
$_['column_stock']  = 'Наличие';
$_['column_price']  = 'Цена';
$_['column_action'] = 'Действие';
?>