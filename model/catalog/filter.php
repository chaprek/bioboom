...
class ModelCatalogFilter extends Model {
	...
	public function getOptionByFilterGroupsId($filter_group_id, $category_id) {
		
		$filter_group_id = (int)$filter_group_id;
		
		$options_data = array();
		
		if ($this->checkGroup($filter_group_id, $category_id)) {
			
			$options_data = $this->cache->get('option.' . $filter_group_id . '.' . $this->config->get('config_language_id'));

			if(!$options_data && !is_array($options_data)) {
			
				$query = $this->db->query("SELECT co.*, cod.*, stl.style_id, tp.type_index	FROM `" . DB_PREFIX . "category_option` co LEFT JOIN `" . DB_PREFIX . "category_option_description` cod ON (co.option_id = cod.option_id) LEFT JOIN `" . DB_PREFIX . "style_option` stl ON (co.option_id = stl.option_id) LEFT JOIN `" . DB_PREFIX . "type_option` tp ON (co.option_id = tp.option_id) WHERE co.option_id IN (SELECT option_id FROM `" . DB_PREFIX . "filter_group_option_to_filter_group` WHERE filter_group_id = " . $filter_group_id . ") AND co.status = 1 AND cod.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY co.sort_order");
				  
				$options_data = $query->rows;  
				
				$this->cache->set('option.' . $filter_group_id . '.' . $this->config->get('config_language_id'), $options_data);
	...
	public function checkGroup ($filter_group_id, $category_id) {
		$query = $this->db->query("SELECT category_id FROM `" . DB_PREFIX . "filter_group_to_category` WHERE filter_group_id = '" . (int)$filter_group_id . "'");
		$group_categories = $query->rows;
		
		if (!empty($group_categories)) {
			$query = $this->db->query("SELECT category_id FROM `" . DB_PREFIX . "filter_group_to_category` WHERE filter_group_id = '" . (int)$filter_group_id . "' AND category_id = '" . $category_id . "'");
			$group_categories = $query->rows;
			if (!empty($group_categories)) {
				return true;
			} else {
	...
	public function getOptionItemNames($filter_options_id, $categories_id) {
		
		$categories_hash = md5($categories_id);
		$cache_data = $this->cache->get('get_option_items_names.' . $filter_options_id . '.' . $categories_hash . '.' . $this->config->get('config_language_id'));
		
		if (!$cache_data && !is_array($cache_data)) {
		
			$query = $this->db->query("SELECT DISTINCT prd.option_value_id as value, prd.option_id as id, opt.name, opv.image FROM `" . DB_PREFIX . "product_option_value` prd LEFT JOIN `" . DB_PREFIX . "option_value_description` as opt ON (opt.option_value_id=prd.option_value_id AND opt.option_value_id=prd.option_value_id) LEFT JOIN `" . DB_PREFIX . "option_value` as opv ON (opv.option_value_id=prd.option_value_id) WHERE prd.option_id IN (" . $this->db->escape($filter_options_id) . ") AND opt.language_id = '" . (int)$this->config->get('config_language_id') . "' AND prd.product_id IN (SELECT product_id FROM `" . DB_PREFIX . "product_to_category` WHERE category_id IN (" . $this->db->escape($categories_id) . "))");
			
			$options = $query->rows;
			$this->cache->get('get_option_items_names.' . $filter_options_id . '.' . $categories_hash . '.' . $this->config->get('config_language_id'), $options);
		}
		else {
			$options = $cache_data;
		}
	 ...
	 public function getAttributeItemNames($filter_attributes_id, $categories_id) {
			
		$query = $this->db->query("SELECT DISTINCT text as value, attribute_id as id, text as name FROM `" . DB_PREFIX . "product_attribute` WHERE attribute_id IN (" . $this->db->escape($filter_attributes_id) . ") AND language_id = '" . (int)$this->config->get('config_language_id') . "' AND product_id IN (SELECT product_id FROM `" . DB_PREFIX . "product_to_category` WHERE category_id IN (" . $this->db->escape($categories_id) . "))");
