...
class ControllerModulesertificates extends Controller {

	private $_name = 'sertificates';
	...
	protected function index($setting) {
		...
		$this->data['sertificates_count'] = $this->model_catalog_sertificates->getTotalsertificates();
		
		$this->data['sertificates_limit'] = $setting['limit'];
	
		if ($this->data['sertificates_count'] > $this->data['sertificates_limit']) { $this->data['showbutton'] = true; } else { $this->data['showbutton'] = false; }
	
		$this->data['buttonlist'] = $this->language->get('buttonlist');
	
		$this->data['sertificateslist'] = $this->url->link('information/sertificates');
		
		$this->data['numchars'] = $setting['numchars'];
		
		if (isset($this->data['numchars'])) { $chars = $this->data['numchars']; } else { $chars = 100; }
		
		$this->data['sertificates'] = array();
	
		$results = $this->model_catalog_sertificates->getsertificatesShorts($setting['limit']);
